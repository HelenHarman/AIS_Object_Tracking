...
{
    /*this->scaleX = ((double)appearance.size().width/(double)frame.size().width)*100.00;
    this->scaleY = ((double)appearance.size().height/(double)frame.size().height)*100.00;
    this->x = 0;//i + ((((double)frame.size().width / 100.00) * this->scaleX)/2);
    this->y = 0;//j + ((((double)frame.size().height / 100.00) * this->scaleY)/2);
    this->rotation = 0;*/
   // Location(frame, appearance.size().width, appearance.size().height, offSetX, offSetY);
...
void Location::getCornerPoint(int *x, int *y, Mat frame)
{
    ...
    *y = this->y - ((((double)frame.size().height / 100.00) * this->scaleY)/2);
    ...
    //*x = this->x - (frame.size().width/2);
    //*y = this->y - (frame.size().height/2);
}
