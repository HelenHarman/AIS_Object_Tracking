    ...
    for node in splitLine :
        if '\n' not in node :
            if nodes[len(nodes)-1] < int(node.strip()):
                links[nodes[len(nodes)-1]].append(getString(int(node.strip()) + linkStartId));
file.close()
...
for node in nodes: # create each node
    ...
    file.write(getString(int(node) + linkStartId) + " [height = 40, width = 40, label = \"\", background = \"" + dirName  + "/" + str(node) + ".png\"];\n")

for key in links: # add the links between the nodes
    for node in links[key]:
        file.write(getString(int(key) + linkStartId) + " -> " + str(node) + "[dir = none];\n") #C -> A;
